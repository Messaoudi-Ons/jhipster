...
export class CommandeService {
  ...
  protected convertDateFromClient(commande: ICommande): ICommande {
    const copy: ICommande = Object.assign({}, commande, {
      dateCom: commande.dateCom && commande.dateCom.isValid() ? commande.dateCom.format(DATE_FORMAT) : undefined,
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.dateCom = res.body.dateCom ? moment(res.body.dateCom) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((commande: ICommande) => {
        commande.dateCom = commande.dateCom ? moment(commande.dateCom) : undefined;
      });
    }
    return res;
  }
